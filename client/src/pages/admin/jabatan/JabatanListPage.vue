<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-surface-900 dark:text-white">Data Jabatan</h1>
    ...
    <div class="card p-6">
      <AppDataTable
        ...
        empty-text="Belum ada data jabatan"
        @page-change="onPageChange"
        @search="onSearch"
      >
        <template #cell-namaJabatan="{ row }">
          <span class="font-medium text-surface-900 dark:text-white">{{ row.namaJabatan }}</span>
          <p v-if="row.deskripsi" class="text-xs text-surface-400 mt-0.5">{{ row.deskripsi }}</p>
        </template>

        <template #cell-gajiPokokDefault="{ row }">
          {{ formatRupiah(row.gajiPokokDefault) }}
        </template>

        <template #cell-tunjanganDefault="{ row }">
          {{ formatRupiah(row.tunjanganDefault) }}
        </template>
    ...
    <!-- Create/Edit Modal -->
    <AppModal v-model="showModal" :title="isEditing ? 'Edit Jabatan' : 'Tambah Jabatan'" size="lg" @close="resetForm">
      <JabatanForm
    ...
    <!-- Delete Confirmation -->
    <AppModal v-model="showDeleteConfirm" title="Hapus Jabatan" size="sm" @close="deletingId = null">
      <div class="text-center py-4">
        ...
        <p class="text-surface-700 dark:text-surface-300 mb-1">Apakah Anda yakin ingin menghapus jabatan ini?</p>
...
import type { JabatanForm as JabatanFormData } from '@/types/jabatan';
...
const editingData = ref<Partial<JabatanFormData>>({});
...
async function handleSubmit(data: JabatanFormData) {
...
async function confirmDelete(jabatan: Jabatan) {
